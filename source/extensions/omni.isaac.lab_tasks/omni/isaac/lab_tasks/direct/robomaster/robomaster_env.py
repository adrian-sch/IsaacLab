...
class RobomasterEnvCfg(DirectRLEnvCfg):
    ...
    decimation = 30 # 10 Hz
    action_scale_lin = 2.0
    action_scale_ang = 6.28
...
class RobomasterEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: RobomasterEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        # Action scaling
        self._action_scale = torch.tensor([self.cfg.action_scale_lin, self.cfg.action_scale_lin, self.cfg.action_scale_ang], device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        actions = torch.mul(actions, self._action_scale)
    ...
    def _get_observations(self) -> dict:
        ...
        dist_to_goal = torch.sqrt(torch.square(self.goal_pos[:, :2] - self._robot.data.root_pos_w[:,:2]).sum(-1)).unsqueeze(1)
        dist_to_goal2 = torch.norm(self.goal_pos - self._robot.data.root_pos_w, dim=-1).unsqueeze(1)

        # TODO goal position relative to robot
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        reward = (self._dist_to_goal_buf - dist_to_goal) * 100.0
    ...
    def _get_dones(self) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        #TODO calulate this more effective and not in every function
        dist_to_goal = torch.sqrt(torch.square(self.goal_pos[:, :2] - self._robot.data.root_pos_w[:,:2]).sum(-1))
